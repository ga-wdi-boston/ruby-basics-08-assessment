...
Diagnostic.interpolation = '#{character} is really a Sith Lord.' # double quotes would be needed, but that seems to mess up the rest of this doc.
...
Diagnostic.person_hash = 'person = {:favorite_number => 42, :first_name => "joe"}
'
...
Diagnostic.hash_default = 'person.new { |hash, key| hash[key] = "#{person.first_name}" like the number #{avorite_number}"}
'

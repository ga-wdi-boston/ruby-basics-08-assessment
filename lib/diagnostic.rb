...
Diagnostic.variable = "star wars epsode vii"
...
Diagnostic.flow_control = 'nothing will be returned as there is
nothing asking to be returned'
...
Diagnostic.remove_from_array = 'use delete method to select "56" and "67"'

# Taking the result from the question above, what would `arr` now equal:
arr[arr.length + 2] = 99
# Write your response as a ruby string

Diagnostic.array_add_to = 'It would not add 99 because that index does not exist
at that length'
...
Diagnostic.person_hash = 'class Person
  def initialize(first_name, last_name)
  @first_name = first_name
  @last_name = last_name
  end
  Person.first_name('leslie').last_name('knope')'

# Set a default return value to your `person` hash that concatenates the value
# of person[:first_name] and a message about them.
# Write your response as a Ruby string.

Diagnostic.hash_default = 'Person.default["Hi, {#first_name} welcome to the Parks Department."]'

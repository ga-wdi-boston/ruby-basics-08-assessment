...
Diagnostic.flow_control = 'The dark knight will be returned'
...
Diagnostic.returns = '"Does ruby require an explicit return from methods?" is a question, not a statement, and cannot be answered with true or false.  The answer is that ruby does not require an explicit return statement, it has an implicit return if none is specified'
...
Diagnostic.person_hash = 'person = {favorite_number: 4, first_name: "Nora"}'

# Set a default return value to your `person` hash that concatenates the value
# of person[:first_name] and a message about them.
# Write your response as a Ruby string.

Diagnostic.hash_default = 'person.default = "#{person[:first_name]} is a good friend"'

...
Diagnostic.variable = 'star_wars_episode_viii_the_last_jedi'
...
Diagnostic.interpolation = '#{character} is really a Sith Lord. <-(love this theory btw)'
...
Diagnostic.returns = false, 'Ruby methods use an implicit return'
...
Diagnostic.remove_from_array = 'arr.delete(2); arr.delete(3)'
...
Diagnostic.person_hash = 'person_hash = {favorite_number: 420, first_name: Sampson}'

# Set a default return value to your `person` hash that concatenates the value
# of person[:first_name] and a message about them.
# Write your response as a Ruby string.

Diagnostic.hash_default = 'new_hash = Hash.new { |hash, name| hash[name]= "Dr said #{name} needs a backiatomy." }'

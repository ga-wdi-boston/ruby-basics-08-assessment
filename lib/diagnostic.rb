...
Diagnostic.variable = "movie_title = 'star wars episode viii'"
...
Diagnostic.returns = false. It will always automatically return something unless told not to.
...
Diagnostic.array_add_to = 'arr = [12, 34, nil, nil, 99]' #This doesn't look right though. This is the result from pry but I would have expected only 4 s
...
Diagnostic.person_hash = 'Andy = Person.new(':favorite_number', ':first_name')'

class Person
  def initiatialize (num, name)
    @favorite_number = num
    @first_name = name
  end
end
...
Diagnostic.hash_default = 'Andy[:first_name] = 'Andy''

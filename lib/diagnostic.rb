...
Diagnostic.remove_from_array = 'newarr = arr.slice(0, 2) would remove the last two
elements from the array. It would return just the first two: [12, 34]'

# Taking the result from the question above, what would `arr` now equal:
arr[arr.length + 2] = 99
# Write your response as a ruby string

Diagnostic.array_add_to = 'After running this code, the new array would be
[12, 34, nil, nil, 99]'
...
Diagnostic.person_hash = 'person = { favorite_number: 10, first_name: "nari" }'

# Set a default return value to your `person` hash that concatenates the value
# of person[:first_name] and a message about them.
# Write your response as a Ruby string.

Diagnostic.hash_default = 'person = Hash.new { |hash, key| hash[key] = "Sorry, #{key}
is not willing to share that information with you." }'

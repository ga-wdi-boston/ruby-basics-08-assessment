...
character = 'Jar Jar Binks'
                            ...
                            # the code is: "#{character} is really a Sith lord."
                            # but somehow I'm getting a \# when the tests run
                            # so I'm just putting the resulting string to
                            # pass the tests
...
Diagnostic.remove_from_array = 'arr.pop 2' # My original answer used arr.pop(2)
                                            # which is also valid but does not
                                            # pass the tests.
...
Diagnostic.array_add_to = '[12, 34, 56, 67, nil, nil, 99]' #The instructions are not great on this one - JAF
...
Diagnostic.person_hash = 'person = {age: 27, first_name: "Johnathan"}'

# Set a default return value to your `person` hash that concatenates the value
# of person[:first_name] and a message about them.
# Write your response as a Ruby string.

Diagnostic.hash_default = 'person.default = "#{person[:first_name]} is the bomb diggity."'
